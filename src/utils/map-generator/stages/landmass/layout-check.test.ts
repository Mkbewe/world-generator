import { isLandmassLayout, validateLayout, validatePlacement } from './layout-check';
...
describe('validatePlacement', () => {
  ...
  function placed(id: string, x: number, y: number): GeologicalStructure {
    return {
      id,
      archetype: 'elongated',
      nodes: [
  ...
  it('reports overlaps and structures hanging outside the world', () => {
    const problems = validatePlacement(
      [placed('a', 0.5, 0.5), placed('b', 0.52, 0.5)],
      inside,
      0.5
    );
    expect(problems.some(problem => problem.includes('overlap'))).toBe(true);
    expect(
      validatePlacement([placed('c', 1.1, 0.5)], inside, 0.5).some(problem =>
        problem.includes('influence')
      )
    ).toBe(true);
    expect(validatePlacement([placed('d', 0.2, 0.5)], inside, 0.5)).toEqual([]);
  });
});
