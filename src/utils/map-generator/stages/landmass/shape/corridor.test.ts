...
describe('landmass topology', () => {
  ...
  it('spans the lagoon from an open arc to a near-ring without ever closing', () => {
    let nearRings = 0;
    for (const seed of SEEDS) {
      ...
      // Open by construction: one edge fewer than nodes, never a closing seam.
      expect(lagoon.edges.length).toBe(lagoon.nodes.length - 1);
      expect(() => validateLayout(layoutFor(lagoon))).not.toThrow();
      if (gap < widest) {
        nearRings++;
      }
    }

    expect(nearRings).toBeGreaterThanOrEqual(3);
  });
